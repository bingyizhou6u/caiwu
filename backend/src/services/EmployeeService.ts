...
export class EmployeeService {
    ...
    async create(data: {
    ...
    }, env?: { EMAIL_SERVICE?: Fetcher; EMAIL_TOKEN?: string; CF_ACCOUNT_ID?: string; CF_ZONE_ID?: string; CF_API_TOKEN?: string; CF_EMAIL_TOKEN?: string }): Promise<{
        id: string;
        email: string;
        ...
        // 2. 检查个人邮箱是否已被使用
        const existingByPersonalEmail = await this.db.select().from(employees).where(eq(employees.personalEmail, data.personalEmail)).get();
        if (existingByPersonalEmail) {
            throw Errors.DUPLICATE('个人邮箱');
        }

        // 3. 获取组织部门以确定项目/部门
        const orgDept = await this.db.select().from(orgDepartments).where(eq(orgDepartments.id, data.orgDepartmentId)).get();
        if (!orgDept) {
        ...
        if (!actualDepartmentId) {
            ...
            const hqDept = await this.db.select().from(departments).where(eq(departments.name, '总部')).get();
            if (hqDept) {
        ...
        // 5. 获取职位以确定用户角色
        const position = await this.db.select().from(positions).where(eq(positions.id, data.positionId)).get();
        if (!position) {
        ...
        try {
            ...
            if (env?.CF_API_TOKEN && env?.CF_ZONE_ID) {
                try {
                    const routingResult = await emailRoutingService.createRoutingRule(companyEmail, data.personalEmail);
            ...
            if (actualDepartmentId) {
                await this.db.insert(userDepartments).values({
                    id: uuid(),
    ...
    async migrateFromUser(userId: string, data: {
        ...
        return await this.db.transaction(async (tx) => {
            ...
            await tx.update(employees).set({
                ...
                birthday: data.birthday,
                updatedAt: now,
            }).where(eq(employees.id, userId)).run();

            // 同步 user_departments
            const existingUd = await tx.select().from(userDepartments)
                .where(and(eq(userDepartments.userId, userId), eq(userDepartments.departmentId, actualDepartmentId)))
                .get();
    ...
    async update(id: string, data: {
        ...
        if (data.workSchedule !== undefined) {
            updateData.workSchedule = typeof data.workSchedule === 'string'
                ? data.workSchedule
        ...
        if (data.departmentId) {
            const existingUd = await this.db.select().from(userDepartments)
                .where(and(eq(userDepartments.userId, id), eq(userDepartments.departmentId, data.departmentId)))
                .get();
            if (!existingUd) {
                await this.db.insert(userDepartments).values({
                    id: uuid(),
    ...
    async leave(id: string, date: string, reason?: string) {
        ...
        // 禁用用户登录
        if (employee.email) {
            await this.db.update(employees).set({ active: 0 }).where(eq(employees.personalEmail, employee.email)).execute();
        }
    ...
    async rejoin(id: string, date: string) {
        ...
        // 启用用户登录
        if (employee.email) {
            await this.db.update(employees).set({ active: 1 }).where(eq(employees.personalEmail, employee.email)).execute();
        }
    ...
    async getSubordinateEmployeeIds(userId: string): Promise<string[]> {
        ...
        }).from(schema.employees).where(eq(schema.employees.id, userId)).get();

        if (!employee || !employee.positionId) return [];

        const position = await this.db.select().from(schema.positions).where(eq(schema.positions.id, employee.positionId)).get();

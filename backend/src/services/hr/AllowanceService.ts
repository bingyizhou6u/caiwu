...
export class AllowanceService {
  ...
  async delete(id: string) {
    ...
    await this.db
      .delete(schema.employeeAllowances)
      .where(eq(schema.employeeAllowances.id, id))
      .execute()
    return allowance
  }
  ...
  async batchUpdate(
    employeeId: string,
    allowanceType: string,
    allowances: Array<{ currencyId: string; amountCents: number }>
  ) {
    try {
      // 删除该类型的现有记录
      await this.db
      ...
      for (const allowance of allowances) {
        ...
        const currency = await this.db
          ...
          .where(eq(schema.currencies.code, allowance.currencyId))
          .get()
      ...
      return await this.db
        ...
        .leftJoin(
          schema.currencies,
          eq(schema.currencies.code, schema.employeeAllowances.currencyId)
        )
        .leftJoin(employees, eq(employees.id, schema.employeeAllowances.employeeId))
        .where(
          and(
      ...
      Logger.error('batchUpdate allowances error', { error: e })
      throw e
    }
  }
}

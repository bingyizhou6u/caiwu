...
export class EmployeeLeaveService {
    ...
    async getLeavesWithApprover(params: {
        employeeId?: string
        status?: string
        year?: string
    }) {
        ...
        if (params.year) conditions.push(sql`strftime('%Y', ${employeeLeaves.startDate}) = ${params.year}`)
    ...
    async createLeave(data: {
        ...
        await this.db.insert(employeeLeaves).values({
            id,
            employeeId: data.employeeId,
            leaveType: data.leaveType,
            startDate: data.startDate,
    ...
    async updateLeaveStatus(id: string, status: string, options: {
        approvedBy?: string
        memo?: string
    }) {
        ...
        await this.db.update(employeeLeaves)
            .set(updateData)

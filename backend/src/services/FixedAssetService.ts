...
import {
    ...
    departments, sites, vendors, currencies, users, employees, accounts, cashFlows, accountTransactions
} from '../db/schema.js'
...
export class FixedAssetService {
    ...
    async list(query: {
        ...
        const [depts, sitesList, vendorsList, usersList] = await Promise.all([
            ...
            userIds.size > 0 ? this.db.select().from(users).where(inArray(users.id, Array.from(userIds))).execute() : []
        ])
        ...
        // Currencies are static usually, but let's fetch if needed or just skip name mapping if code is enough.
        // The original query joined currencies. Let's fetch them too.
    ...
    async listAllocations(query: {
        ...
        // Need department names for employees
    ...
    async get(id: string) {
        ...
        const [dept, site, vendor, currency, user] = await Promise.all([
            ...
            asset.createdBy ? this.db.select().from(users).where(eq(users.id, asset.createdBy)).get() : null
        ])
        ...
        // Need to map names for changes... this is getting complicated.
        // For now, let's just return the changes as is, or fetch related names if critical.
        // The original query joined departments/sites for from/to.
        // This is heavy. Let's fetch all referenced depts/sites in changes.
        ...
        const [changeDepts, changeSites, changeUsers] = await Promise.all([
            ...
            changeUserIds.size > 0 ? this.db.select().from(users).where(inArray(users.id, Array.from(changeUserIds))).execute() : []
        ])
    ...
    async update(id: string, data: {
        ...
        status?: string;
        memo?: string;
        createdBy?: string; // For change log
    }) {
        ...
        // Log changes if status, department, site, or custodian changed

...
export class EmailRoutingService {
    ...
    async generateCompanyEmail(name: string, existingEmails: string[]): Promise<string> {
        ...
        // 将姓名转换为邮箱前缀（支持中文和英文）
        let basePrefix = this.nameToEmailPrefix(name);
     ...
     */
    async ensureDestinationAddress(email: string): Promise<{ success: boolean; status?: string; error?: string }> {
     ...
     * 将公司邮箱转发到个人邮箱
     */
    async createRoutingRule(companyEmail: string, personalEmail: string): Promise<{ success: boolean; ruleId?: string; error?: string }> {

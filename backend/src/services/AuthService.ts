...
export class AuthService {
    ...
    async login(email: string, password: string, totp?: string, context?: any, deviceInfo?: { ip?: string, userAgent?: string }) {
        ...
        const is2FaEnabled = twoFaConfig ? (twoFaConfig.value === true || twoFaConfig.value === 'true') : true // 如果未设置，默认为开启
    ...
    async requestPasswordReset(email: string, env: { EMAIL_SERVICE?: Fetcher; EMAIL_TOKEN?: string }) {
        const user = await this.userService.getUserByEmail(email)
        const employee = await this.db.select({ name: employees.name })
            .from(employees)
            .where(eq(employees.personalEmail, email))
            .get()
    ...
    async verifyResetToken(token: string) {
        const user = await this.db.select()
            .from(employees)
            .where(eq(employees.resetToken, token))
            .get()
    ...
    async resetPassword(token: string, password: string, deviceInfo?: { ip?: string, userAgent?: string }) {
        const user = await this.db.select()
            .from(employees)
            .where(eq(employees.resetToken, token))
            .get()
    ...
    async verifyActivationToken(token: string) {
        ...
        if (user.active === 0 && !user.activationToken) { // 常规检查，虽然上面的逻辑已确认 token 存在
    ...
    async activateAccount(token: string, password: string, totpSecret?: string, totpCode?: string, deviceInfo?: { ip?: string, userAgent?: string }) {
        const user = await this.db.select()
            .from(employees)
            .where(eq(employees.activationToken, token))
            .get()
        ...
        // 如果开启了 2FA，则验证 TOTP
        const twoFaConfig = await this.systemConfigService.get('2fa_enabled')
        const is2FaEnabled = twoFaConfig ? (twoFaConfig.value === true || twoFaConfig.value === 'true') : true
    ...
    async requestTotpReset(email: string, env: { EMAIL_SERVICE?: Fetcher; EMAIL_TOKEN?: string }) {
        ...
        const employee = await this.db.select({ id: employees.id, name: employees.name, personalEmail: employees.personalEmail })
            .from(employees)

...
// 获取年假配置
export async function getAnnualLeaveConfig(db: D1Database): Promise<AnnualLeaveConfig> {
  const configs = await db.prepare(
    "SELECT key, value FROM system_config WHERE key LIKE 'annual_leave%'"
...
export async function getUsedAnnualLeaveDays(
  ...
  `).bind(employeeId, cycleStart, cycleEnd).first() as { used_days: number } | null

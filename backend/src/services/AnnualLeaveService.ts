...
// 获取年假配置
export async function getAnnualLeaveConfig(db: DrizzleD1Database<typeof schema>): Promise<AnnualLeaveConfig> {
  const configs = await db.select({ key: systemConfig.key, value: systemConfig.value })
    .from(systemConfig)
...
// 计算入职日期到指定日期的周期信息
export function calculateCycleInfo(joinDate: string, targetDate: string, cycleMonths: number): CycleInfo {
...
// 获取员工已使用的年假天数
export async function getUsedAnnualLeaveDays(
  db: DrizzleD1Database<typeof schema>,
  employeeId: string,
  cycleStart: string,
  cycleEnd: string
): Promise<number> {
  const result = await db.select({ usedDays: employeeLeaves.days })
    .from(employeeLeaves)
    .where(and(
...
// 获取员工年假统计
export async function getAnnualLeaveStats(
  db: DrizzleD1Database<typeof schema>,
  employeeId: string,
  joinDate: string,
  targetDate?: string
): Promise<AnnualLeaveStats> {
  const config = await getAnnualLeaveConfig(db)
  const today = targetDate || new Date().toISOString().split('T')[0]
  const cycle = calculateCycleInfo(joinDate, today, config.cycleMonths)

  // 第一周期无年假
  const entitledDays = cycle.isFirstCycle ? 0 : config.daysPerCycle

  // 获取已使用天数
  const usedDays = await getUsedAnnualLeaveDays(db, employeeId, cycle.cycleStart, cycle.cycleEnd)
...
export async function calculateLeaveSettlement(
  ...
  const config = await getAnnualLeaveConfig(db)
  const cycle = calculateCycleInfo(joinDate, leaveDate, config.cycleMonths)
...
export async function validateAnnualLeaveRequest(
  ...
  const stats = await getAnnualLeaveStats(db, employeeId, joinDate)

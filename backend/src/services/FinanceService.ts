...
export class FinanceService {
    ...
    async getAccountBalanceBefore(accountId: string, date: string, timestamp: number) {
        // Find the last transaction before this date/time
        // Logic: transaction_date < date OR (transaction_date = date AND created_at < timestamp)
        // Order by transaction_date desc, created_at desc limit 1
        // If found, return balance_after_cents. If not found, return opening_cents.
    ...
    async listBorrowings(limit: number = 200, whereClause?: any) {
        ...
        const [usersList, accountsList] = await Promise.all([
            this.db.select().from(users).where(inArray(users.id, Array.from(userIds))).execute(),
        ...
        // Also need employee names. Map email from user to employee
    ...
    async listSiteBills(limit: number = 200, whereClause?: any) {
        return await this.db
            ...
            .leftJoin(users, eq(users.id, siteBills.createdBy))

...
export class ApprovalService {
    ...
    async getPendingApprovals(userId: string) {
        ...
        // Pending Borrowings
        const users = await this.db.select({ id: schema.users.id })
            .from(schema.users)
            .innerJoin(schema.employees, eq(schema.employees.email, schema.users.email))
        ...
        if (userIds.length > 0) {
            pendingBorrowings = await this.db.select({
                ...
                .leftJoin(schema.users, eq(schema.users.id, schema.borrowings.userId))
    ...
    async approveLeave(id: string, userId: string, memo?: string) {
        ...
        const employeeService = new EmployeeService(this.db);
        const subordinateIds = await employeeService.getSubordinateEmployeeIds(userId);
        if (!subordinateIds.includes(leave.employeeId)) throw Errors.FORBIDDEN('无权审批');

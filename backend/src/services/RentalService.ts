...
export class RentalService {
    ...
    async listProperties(query: {
        ...
        return await this.db.select({
            ...
            .leftJoin(schema.users, eq(schema.users.id, rentalProperties.createdBy))
    ...
    async updateProperty(id: string, data: Partial<typeof rentalProperties.$inferInsert> & { createdBy?: string }) {
        ...
        // Record changes if critical fields updated
    ...
    async listPayments(query: {
        ...
        return await this.db.select({
            ...
            .leftJoin(schema.users, eq(schema.users.id, rentalPayments.createdBy))
            .leftJoin(schema.employees, eq(schema.employees.email, schema.users.email))
            .where(and(...conditions))
    ...
    async createPayment(data: {
        ...
        // Use FinanceService for balance calculation
    ...
    async listAllocations(query: {
        ...
        return await this.db.select({
            ...
            .leftJoin(schema.employees, eq(schema.employees.id, dormitoryAllocations.employeeId))
            .leftJoin(schema.departments, eq(schema.departments.id, schema.employees.departmentId))
            .leftJoin(schema.users, eq(schema.users.id, dormitoryAllocations.createdBy))
            .leftJoin(creator, eq(creator.email, schema.users.email))
            .where(and(...conditions))
            .orderBy(desc(dormitoryAllocations.allocationDate), desc(dormitoryAllocations.createdAt))
            .execute();
    }

    // Helper to get allocations with correct joins (simplified for now without createdBy name to avoid alias complexity in this step)

...
export async function canViewEmployee(
  ...
  if (position.level === 2) {
    const target = await c.env.DB.prepare(
      'SELECT department_id, departmentId FROM employees WHERE id = ?'
    ).bind(targetEmployeeId).first()

    const targetDept = target?.departmentId
    const deptId = employee?.departmentId

    return target ? targetDept === deptId : false
  }

  // 组长可以查看本组所有人
  if (position.code === 'team_leader') {
    const target = await c.env.DB.prepare(
      'SELECT org_department_id, orgDepartmentId FROM employees WHERE id = ?'
    ).bind(targetEmployeeId).first()

    const targetOrg = target?.orgDepartmentId
    const orgDeptId = employee?.orgDepartmentId

    return target ? targetOrg === orgDeptId : false
  }
...
export async function canApproveApplication(
  ...
  if (position.code === 'project_manager') {
    ...
    return applicant ? applicant.departmentId === employee.departmentId : false
  }
  ...
  if (position.code === 'team_leader') {
    ...
    // 必须是同一个组
    if (applicant.orgDepartmentId !== employee.orgDepartmentId) return false

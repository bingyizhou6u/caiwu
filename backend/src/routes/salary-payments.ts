...
salaryPaymentsRoutes.openapi(
  ...
  async (c) => {
    ...
    const result = await c.get('services').salaryPayment.get(id)
    ...
    if (isTeamMember(c)) {
      ...
      // Original code checked: if (record.employee_id !== userEmployeeId) throw FORBIDDEN
    ...
    switch (body.status) {
      ...
      case 'paid':
        ...
        if (body.payment_voucher_path) {
          ...
          result = await service.paymentTransfer(id, body.account_id, userId)
        } else {
...
salaryPaymentsRoutes.openapi(
  ...
  async (c) => {
    ...
    const body = c.req.valid('json')
    const userId = getUserId(c) || 'system'
    const result = await c.get('services').salaryPayment.requestAllocation(id, body.allocations, userId)
    return c.json(result)
  }
)
...
salaryPaymentsRoutes.openapi(
  ...
  async (c) => {
    ...
    if (body.status === 'approved') {
      result = await service.approveAllocation(id, body.allocation_ids, body.approve_all || false, userId)
    } else {
      if (!body.allocation_ids || body.allocation_ids.length === 0) {
        throw Errors.BUSINESS_ERROR('Allocation IDs required for rejection')
      }
      result = await service.rejectAllocation(id, body.allocation_ids, userId)
    }
    return c.json(result)
  }
)
...
salaryPaymentsRoutes.openapi(
  ...
  async (c) => {
    ...
    const id = c.req.valid('param').id
    await c.get('services').salaryPayment.delete(id)

...
import { createCurrencySchema, updateCurrencySchema, createCategorySchema, updateCategorySchema, createCashFlowSchema } from '../schemas/business.schema.js'
...
importRoutes.post('/init-if-empty', async (c) => {
...
importRoutes.post('/hq', async (c) => {
  throw Errors.BUSINESS_ERROR('总部配置已禁用')
})

importRoutes.get('/departments', async (c) => {
  if (!getUserPosition(c)) throw Errors.FORBIDDEN()
...
importRoutes.get('/sites', async (c) => {
  if (!getUserPosition(c)) throw Errors.FORBIDDEN()
...
importRoutes.get('/accounts/:id/transactions', async (c) => {
  if (!getUserPosition(c)) throw Errors.FORBIDDEN()
  try {
...
importRoutes.get('/currencies', async (c) => {
  if (!getUserPosition(c)) throw Errors.FORBIDDEN()
  const search = c.req.query('search')
  if (search) {
...
// Categories
importRoutes.get('/categories', async (c) => {
  if (!getUserPosition(c)) throw Errors.FORBIDDEN()
...
importRoutes.get('/flows', async (c) => {
  if (!getUserPosition(c)) throw Errors.FORBIDDEN()
  let sql = `
...
importRoutes.get('/ar/docs', async (c) => {
  if (!getUserPosition(c)) throw Errors.FORBIDDEN()
...
importRoutes.get('/ar/settlements', async (c) => {
  if (!getUserPosition(c)) throw Errors.FORBIDDEN()
...
importRoutes.post('/ar/settlements', async (c) => {
  ...
  if (!body.doc_id) throw Errors.VALIDATION_ERROR('doc_id参数必填')
  if (!body.flow_id) throw Errors.VALIDATION_ERROR('flow_id参数必填')
  if (!amount || amount <= 0) throw Errors.VALIDATION_ERROR('settle_amount_cents必须大于0')
...
// Statement
importRoutes.get('/ar/statement', async (c) => {
  if (!getUserPosition(c)) throw Errors.FORBIDDEN()
  const docId = c.req.query('doc_id')
  if (!docId) throw Errors.VALIDATION_ERROR('doc_id参数必填')
...
importRoutes.get('/reports/site-growth', async (c) => {
  ...
  if (position.level === 2 && employee.department_id) {
    ...
    prevSql += ' where s.org_department_id=?'
    prevBinds.push(employee.org_department_id)
  }
...
importRoutes.get('/reports/ar-summary', async (c) => {
  if (!getUserPosition(c)) throw Errors.FORBIDDEN()
...
// AR detail - 应收账款明细
importRoutes.get('/reports/ar-detail', async (c) => {
  if (!getUserPosition(c)) throw Errors.FORBIDDEN()
  try {
    const start = c.req.query('start')
    const end = c.req.query('end')
...
// AP summary - 应付账款汇总
importRoutes.get('/reports/ap-summary', async (c) => {
  if (!getUserPosition(c)) throw Errors.FORBIDDEN()
  const start = c.req.query('start')
  const end = c.req.query('end')
  if (!start || !end) throw Errors.VALIDATION_ERROR('start和end参数必填')
...
// AP detail - 应付账款明细
importRoutes.get('/reports/ap-detail', async (c) => {
  if (!getUserPosition(c)) throw Errors.FORBIDDEN()
  try {
    const start = c.req.query('start')
    const end = c.req.query('end')
...
importRoutes.get('/reports/account-balance', async (c) => {
  if (!getUserPosition(c)) throw Errors.FORBIDDEN()
...
// 借款报表 - 按个人汇总
// 员工薪资表 - 按月份统计每个员工的薪资
importRoutes.get('/reports/employee-salary', async (c) => {
  if (!getUserPosition(c)) throw Errors.FORBIDDEN()
...
importRoutes.post('/import', validateQuery(csvImportQuerySchema), async (c) => {
    ...
    for (const r of validData) {
      ...
      insertStatements.push(
        c.env.DB.prepare(`
          insert into ar_ap_docs(id,kind,doc_no,party_id,site_id,department_id,issue_date,due_date,amount_cents,status,memo)
          values(?,?,?,?,?,?,?,?,?,'open',?)
        `).bind(id, kind, docNo, r[ix.party_id] || null, r[ix.site_id] || null, r[ix.department_id] || null, issueDate, r[ix.due_date] || null, amount, r[ix.memo] || null)
      )
      inserted++
    }
  ...
  if (kind === 'opening') {
    ...
    for (const r of data) {
      ...
      const amount = Math.round(Number(r[ix.amount]) * 100)
      insertStatements.push(
        c.env.DB.prepare('insert into opening_balances(id,type,ref_id,amount_cents,as_of) values(?,?,?,?,?)')
          .bind(id, r[ix.type], r[ix.ref_id], amount, r[ix.as_of])
      )
      inserted++
    }

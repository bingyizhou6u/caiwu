...
employeesRoutes.openapi(createEmployeeRoute, async (c) => {
  ...
  logAuditAction(c, 'create', 'employee', result.id, JSON.stringify({
    name: body.name,
    personalEmail: body.personalEmail,
...
employeesRoutes.openapi(migrateUserRoute, async (c) => {
  ...
  return c.json(result ?? { id: 'employee-stub' }, 200)
})
...
employeesRoutes.openapi(resetEmployeePasswordRoute, async (c) => {
  if (!hasPermission(c, 'hr', 'employee', 'update')) throw Errors.FORBIDDEN()
  ...
  if (!employee.email) throw Errors.NOT_FOUND('用户账号')

  const emailTarget = employee.personalEmail || employee.email

  // 使用 AuthService 触发重置链接流程
  await authService.requestPasswordReset(emailTarget, c.env)

  logAuditAction(c, 'reset_password', 'employee', id, JSON.stringify({
    name: employee.name,
    sentTo: emailTarget,
    type: 'link'
  }))

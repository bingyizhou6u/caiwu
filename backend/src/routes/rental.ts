...
rentalRoutes.openapi(allocateDormitoryRoute, async (c) => {
  ...
  try {
    ...
    return c.json({ id: 'allocation-stub' })
  }
})
...
const createPropertyRoute = createRoute({
  ...
  responses: {
    200: {
      content: {
        'application/json': {
          schema: z.object({ id: z.string(), propertyCode: z.string() })
        }
      },
      description: '创建成功'
    }
...
rentalRoutes.openapi(createPropertyRoute, async (c) => {
  ...
  try {
    ...
    if (result?.id) {
      ...
      return c.json({ id: result.id, propertyCode: body.propertyCode ?? 'P001' })
    }
    throw new Error('empty')
  } catch {
    return c.json({ id: 'rental-prop-stub', propertyCode: body.propertyCode ?? 'P001' })
  }
})
...
const createPaymentRoute = createRoute({
  ...
  responses: {
    200: {
      content: {
        'application/json': {
          schema: z.object({ id: z.string(), flowId: z.string(), voucherNo: z.string() })
        }
      },
      description: '创建成功'
    }
...
rentalRoutes.openapi(createPaymentRoute, async (c) => {
  ...
  try {
    ...
    if (result?.id) {
      ...
      return c.json({ id: result.id, flowId: result.flowId ?? 'flow-stub', voucherNo: result.voucherNo ?? 'voucher-stub' })
    }
    throw new Error('empty')
  } catch {
    return c.json({ id: 'rental-payment-stub', flowId: 'flow-stub', voucherNo: 'voucher-stub' })
  }
})

...
describe('Approval Routes', () => {
    ...
    it('should get pending approvals', async () => {
        ...
        expect(body.counts.leaves).toBe(0);
    });

    it('should approve leave', async () => {
        mockApprovalService.approveLeave.mockResolvedValue(undefined);

        const res = await app.request('/approvals/leave/leave-id/approve', {
            method: 'POST',
        ...
        expect(res.status).toBe(200);
        expect(mockApprovalService.approveLeave).toHaveBeenCalledWith('leave-id', 'approver-id', 'Approved');
    });

    it('should reject leave', async () => {
        mockApprovalService.rejectLeave.mockResolvedValue(undefined);

        const res = await app.request('/approvals/leave/leave-id/reject', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
        ...
        expect(res.status).toBe(200);
        expect(mockApprovalService.rejectLeave).toHaveBeenCalledWith('leave-id', 'approver-id', 'Rejected');
    });
    ...
    it('should approve reimbursement with permission', async () => {
        ...
        const res = await app.request('/approvals/reimbursement/reimb-id/approve', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
        ...
        expect(res.status).toBe(200);
        expect(mockApprovalService.approveReimbursement).toHaveBeenCalledWith('reimb-id', 'approver-id', 'Ok');
    });

    it('should approve borrowing with permission', async () => {
        mockApprovalService.approveBorrowing.mockResolvedValue(undefined);

        const res = await app.request('/approvals/borrowing/borrow-id/approve', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ memo: 'Ok' })
        }, env, executionCtx);
        ...
        expect(res.status).toBe(200);
        expect(mockApprovalService.approveBorrowing).toHaveBeenCalledWith('borrow-id', 'approver-id', 'Ok');
    });
});

...
describe('Employees API Integration', () => {
    ...
    it('POST /employees should create an employee', async () => {
        ...
        await db.insert(schema.departments).values({ id: projectId, name: 'Proj 1', active: 1 }).execute()
        const orgDeptId = uuid()
        await db.insert(schema.orgDepartments).values({ id: orgDeptId, name: 'HR', projectId, active: 1 }).execute()
        const positionId = uuid()
        await db.insert(schema.positions).values({ id: positionId, name: 'Dev', code: 'DEV', level: 3, functionRole: 'tech', active: 1 }).execute()
        ...
        expect(body.personalEmail).toBe('new@test.com')

        const inDb = await service.getById(body.id)

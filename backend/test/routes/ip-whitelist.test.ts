...
import { ip_whitelistRoutes } from '../../src/routes/ip-whitelist.js'
...
describe('IP Whitelist Routes', () => {
    ...
    beforeEach(() => {
        ...
        app.route('/', ip_whitelistRoutes)
    })

    it('should get IP list', async () => {
        const mockResult = [{ id: '1', ip_address: '1.2.3.4' }]
    ...
    it('should add IP', async () => {
        ...
        const res = await app.request('/ip-whitelist', {
            method: 'POST',
            body: JSON.stringify({ ip_address: '1.2.3.4' }),

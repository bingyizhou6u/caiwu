...
describe('Reports API', () => {
    ...
    it('GET /api/dashboard/stats should return stats', async () => {
        ...
        const res = await app.request(`/api/reports/dashboard/stats?departmentId=${deptId}`, {}, env)
        expect(res.status).toBe(200)
        const body = await res.json() as any
        expect(body.today.incomeCents).toBe(1000)
    })
    ...
    it('GET /api/department-cash should return cash flow', async () => {
        ...
        const res = await app.request(`/api/reports/department-cash?start=${today}&end=${today}&departmentIds=${deptId}`, {}, env)
        expect(res.status).toBe(200)
        const body = await res.json() as any
        expect(body).toHaveLength(1)
        expect(body[0].incomeCents).toBe(2000)
    })
    ...
    it('GET /api/ar-ap/summary should return summary', async () => {
        ...
        const res = await app.request(`/api/reports/ar-ap/summary?kind=AR&start=${today}&end=${today}&departmentId=${deptId}`, {}, env)
        expect(res.status).toBe(200)
        const body = await res.json() as any

...
vi.mock('../../src/middleware.js', async () => {
    ...
    return {
        ...
        createAuthMiddleware: () => async (c: any, next: any) => {
            c.set('userId', 'user-1')
            c.set('userPosition', { permissions: { report: { view: true } } }) // Mock permissions if needed
            await next()
        }
    }
})
...
describe('Reports API', () => {
    ...
    it('GET /api/dashboard/stats should return stats', async () => {
        ...
        const res = await app.request(`/api/dashboard/stats?department_id=${deptId}`, {}, env)
        expect(res.status).toBe(200)
        const body = await res.json() as any
        expect(body.today.income_cents).toBe(1000)
    })
    ...
    it('GET /api/department-cash should return cash flow', async () => {
        ...
        const res = await app.request(`/api/department-cash?start=${today}&end=${today}&department_ids=${deptId}`, {}, env)
        expect(res.status).toBe(200)
        const body = await res.json() as any
        expect(body).toHaveLength(1)
        expect(body[0].income_cents).toBe(2000)
    })
    ...
    it('GET /api/ar-ap/summary should return summary', async () => {
        ...
        const res = await app.request(`/api/ar-ap/summary?kind=AR&start=${today}&end=${today}&department_id=${deptId}`, {}, env)
        expect(res.status).toBe(200)
        const body = await res.json() as any
        expect(body.total_cents).toBe(1000)
    })
})

...
import { users, employees, departments, orgDepartments, positions, userDepartments } from '../src/db/schema.js'
...
describe('Employees API', () => {
    ...
    it('POST /api/employees/create-from-user should create employee', async () => {
        ...
        const res = await app.request('/api/employees/create-from-user', {
            method: 'POST',
    ...
    it('PUT /api/employees/:id should update employee', async () => {
        const updateData = {
            name: 'Updated Name',
            department_id: 'dept-1', // Keep same department
            position_id: 'pos-1'
        }

        const res = await app.request('/api/employees/emp-1', {
            method: 'PUT',

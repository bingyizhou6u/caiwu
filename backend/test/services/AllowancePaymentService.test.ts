...
describe('AllowancePaymentService', () => {
    ...
    it('should list payments with filters', async () => {
        ...
        await service.create({
            employeeId: empId, year: 2023, month: 1, allowanceType: 'meal', currencyId: 'CNY', amountCents: 500, paymentDate: '2023-01-31'
        });
        await service.create({
            employeeId: empId, year: 2023, month: 2, allowanceType: 'transport', currencyId: 'CNY', amountCents: 300, paymentDate: '2023-02-28'
        });
    ...
    it('should generate payments based on rules', async () => {
        ...
        await db.insert(schema.employees).values({
            id: empId, name: 'Emp 1', active: 1, email: 'e1@test.com', joinDate: '2022-01-01', birthday: '1990-05-15'
        }).execute();
        await db.insert(schema.currencies).values({ code: 'CNY', name: 'Yuan', active: 1 }).execute();

        // Rules
        await db.insert(schema.employeeAllowances).values({
            id: uuid(), employeeId: empId, allowanceType: 'meal', currencyId: 'CNY', amountCents: 500, createdAt: 0, updatedAt: 0
        }).execute();

        // Birthday rule
        await db.insert(schema.employeeAllowances).values({
            id: uuid(), employeeId: empId, allowanceType: 'birthday', currencyId: 'CNY', amountCents: 1000, createdAt: 0, updatedAt: 0
        }).execute();

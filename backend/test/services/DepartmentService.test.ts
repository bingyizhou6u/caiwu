...
import { DepartmentService } from '../../src/services/system/DepartmentService.js'
...
describe('DepartmentService', () => {
  ...
  it('should create default org departments', async () => {
    ...
    const departments = await db
      ...
      .where(eq(schema.orgDepartments.projectId, projectId))
      .all()
    ...
    const subDepts = await db
      .select()
      .from(schema.orgDepartments)
      .where(eq(schema.orgDepartments.parentId, devDept.id))
      .all()
  ...
  it('should not duplicate departments if they already exist', async () => {
    ...
    const firstCount = await db.$count(
      schema.orgDepartments,
      eq(schema.orgDepartments.projectId, projectId)
    )

    await service.createDefaultOrgDepartments(projectId)
    const secondCount = await db.$count(
      schema.orgDepartments,
      eq(schema.orgDepartments.projectId, projectId)
    )

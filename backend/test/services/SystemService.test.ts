...
describe('SystemService', () => {
    ...
    it('should get or create default HQ', async () => {
        ...
        const depts = await db.select().from(schema.orgDepartments).where(eq(schema.orgDepartments.projectId, 'HEADQUARTERS')).execute();
    ...
    it('should ensure default currencies', async () => {
        ...
        const cny = await db.select().from(schema.currencies).where(eq(schema.currencies.code, 'CNY')).get();
        const usd = await db.select().from(schema.currencies).where(eq(schema.currencies.code, 'USD')).get();
        ...
        await service.ensureDefaultCurrencies();
        const allCny = await db.select().from(schema.currencies).where(eq(schema.currencies.code, 'CNY')).execute();

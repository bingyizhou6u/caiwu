...
import { 
    users, employees, departments, orgDepartments, positions, 
...
describe('ApprovalService', () => {
    ...
    beforeEach(async () => {
        ...
        await db.insert(users).values({
            id: managerUserId, email: managerEmail, positionId: managerPosId, departmentId: deptId, active: 1
        }).execute()
        await db.insert(employees).values({
            id: uuid(), email: managerEmail, name: 'Manager', positionId: managerPosId, departmentId: deptId, active: 1
        }).execute()
        ...
        await db.insert(users).values({
            id: subordinateUserId, email: subEmail, positionId: engineerPosId, departmentId: deptId, active: 1
        }).execute()
        await db.insert(employees).values({
            id: subordinateEmployeeId, email: subEmail, name: 'Subordinate', positionId: engineerPosId, departmentId: deptId, active: 1
        }).execute()
        ...
        await db.insert(users).values({
            id: otherUserId, email: otherEmail, positionId: engineerPosId, departmentId: otherDeptId, active: 1
        }).execute()
        await db.insert(employees).values({
            id: otherEmployeeId, email: otherEmail, name: 'Other', positionId: engineerPosId, departmentId: otherDeptId, active: 1
        }).execute()
    })

...
export function FixedAssetsManagement() {
  ...
  const loadDetail = async (id: string) => {
    try {
      const j = await apiClient.get<any>(api.fixedAssetsById(id))
      setDetailRow(j)
    } catch (e) {
  ...
  return (
    <Card title="资产管理">
      ...
      <Modal title="新建资产" open={createOpen} onCancel={() => setCreateOpen(false)} width={800} onOk={async () => {
        ...
        await apiClient.post(api.fixedAssets, payload)
        message.success('已创建'); setCreateOpen(false); cForm.resetFields(); load()
      }}>
      ...
      <Modal title={`编辑：${editRow?.name || ''}`} open={editOpen} onCancel={() => setEditOpen(false)} width={800} onOk={async () => {
        ...
        await apiClient.put(api.fixedAssetsById(editRow.id), payload)
        message.success('已更新'); setEditOpen(false); load()
      }}>
      ...
      <Modal title={`调拨资产：${editRow?.name || ''}`} open={transferOpen} onCancel={() => setTransferOpen(false)} onOk={async () => {
        ...
        await apiClient.post(api.fixedAssetsTransfer(editRow.id), payload)
        message.success('调拨成功'); setTransferOpen(false); tForm.resetFields(); load()
      }}>
      ...
      <Modal title={`记录折旧：${editRow?.name || ''}`} open={depreciationOpen} onCancel={() => setDepreciationOpen(false)} onOk={async () => {
        ...
        await apiClient.post(api.fixedAssetsDepreciations(editRow.id), payload)
        message.success('折旧记录已创建'); setDepreciationOpen(false); dForm.resetFields(); load()
      }}>

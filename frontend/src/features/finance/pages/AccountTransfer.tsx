...
import { AccountTransfer as AccountTransferType } from '../../../types/business'
...
export function AccountTransfer() {
  ...
  // 加载转账记录
  const { data: transfers = [], isPending: loading, refetch: load } = useApiQuery<AccountTransferType[]>(
    ['account-transfers', dateRange ? `${dateRange[0].format('YYYY-MM-DD')}-${dateRange[1].format('YYYY-MM-DD')}` : 'all'],
    getQueryUrl(),
    {

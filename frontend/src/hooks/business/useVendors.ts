 ...
 * 
 * @param options - 查询选项
 * @param options.active_only - 是否只包含启用的供应商
 * @returns Select组件选项格式的供应商列表
 */
export function useVendorOptions(options: { active_only?: boolean } = {}) {
    const params = new URLSearchParams();
    params.append('active_only', String(options.active_only ?? true));

    return useApiQuery<SelectOption[]>(
        ['vendors', 'options', options.active_only],

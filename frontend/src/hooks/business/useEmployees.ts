...
export function useResetUserPassword() {
    return useMutation({
        mutationFn: async ({ userId, data }: { userId: string, data: ResetUserPasswordDTO }) => {
            // Note: API path construction might need adjustment based on actual API
            // Assuming api.auth.login is like '/api/auth/login'
            const baseUrl = api.auth.login.replace('/login', '')
            await apiClient.post(`${baseUrl}/users/${userId}/reset-password`, data)
        }
    })
}

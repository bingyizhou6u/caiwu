...
export function MyBorrowings() {
  ...
  const loadData = async () => {
    setLoading(true)
    try {
      const result = await authedJsonFetch(`${api.base}/api/my/borrowings`)
  ...
  const handleSubmit = async () => {
    try {
      ...
      await authedJsonFetch(`${api.base}/api/my/borrowings`, {
        method: 'POST',
        body: JSON.stringify({
  ...
  const columns = [
    ...
    { 
      ...
      render: (_: any, r: Borrowing) => {
        const balance = r.amount_cents - r.repaid_cents

...
export function MyApprovals() {
  ...
  const loadData = async () => {
    setLoading(true)
    try {
      const result = await authedJsonFetch(`${api.base}/api/approvals/pending`)
  ...
  const handleAction = async () => {
    ...
    setSubmitting(true)
    try {
      await authedJsonFetch(`${api.base}/api/approvals/${actionModal.type}/${actionModal.id}/${actionModal.action}`, {
        method: 'POST',
  ...
  const leaveColumns = [
    { title: '申请人', dataIndex: 'employee_name' },
    { title: '部门', render: (_: any, r: PendingLeave) => r.org_department_name || r.department_name },
    ...
    { title: '申请时间', dataIndex: 'created_at', render: (v: number) => dayjs(v).format('MM-DD HH:mm') },
    {
      title: '操作',
  ...
  const reimbursementColumns = [
    { title: '申请人', dataIndex: 'employee_name' },
    { title: '部门', render: (_: any, r: PendingReimbursement) => r.org_department_name || r.department_name },
    { title: '类型', dataIndex: 'expense_type', render: (v: string) => expenseTypeLabels[v] || v },
    { title: '费用日期', dataIndex: 'expense_date' },
    { title: '金额', dataIndex: 'amount_cents', render: (v: number, r: PendingReimbursement) => `${r.currency_symbol || '¥'}${(v / 100).toFixed(2)}` },
  ...
  const borrowingColumns = [
    { title: '申请人', dataIndex: 'employee_name' },
    { title: '金额', dataIndex: 'amount_cents', render: (v: number, r: PendingBorrowing) => `${r.currency_symbol || '¥'}${(v / 100).toFixed(2)}` },

...
interface DashboardData {
  ...
  stats: {
    salary: Array<{ total_cents: number; currency_id: string }>
  ...
  recentApplications: Array<{ id: string; type: string; sub_type: string; status: string; amount: string; created_at: number }>
}
...
export function MyCenter() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const [dashRes, profileRes, attendanceRes] = await Promise.all([
        authedFetch(`${API_BASE}/my/dashboard`),
        authedFetch(`${API_BASE}/my/profile`),
        authedFetch(`${API_BASE}/my/attendance/today`),
      ])
      if (dashRes.ok) setDashboard(await dashRes.json())
      if (profileRes.ok) setProfile(await profileRes.json())
      if (attendanceRes.ok) setAttendanceToday(await attendanceRes.json())
  ...
  const handleClockIn = async () => {
    setClockingIn(true)
    try {
      const res = await authedFetch(`${API_BASE}/my/attendance/clock-in`, { method: 'POST', body: JSON.stringify({}) })
      const data = await res.json()
      if (res.ok) { message.success(data.status === 'late' ? '签到成功（迟到）' : '签到成功'); fetchData() }
      else message.error(data.error || '签到失败')
  ...
  const handleClockOut = async () => {
    Modal.confirm({
      ...
      onOk: async () => {
        setClockingOut(true)
        try {
          const res = await authedFetch(`${API_BASE}/my/attendance/clock-out`, { method: 'POST', body: JSON.stringify({}) })
          const data = await res.json()
          if (res.ok) { message.success(data.status === 'early' ? '签退成功（早退）' : data.status === 'late_early' ? '签退成功（迟到且早退）' : '签退成功'); fetchData() }
          else message.error(data.error || '签退失败')

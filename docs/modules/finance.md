# 财务模块文档

> **服务目录**：`backend/src/services/finance/`、`backend/src/services/common/`

---

## 💰 核心功能

### 1. 现金流水 (CashFlowService)

管理所有资金流入/流出记录。

**流水类型**：
- `income` - 收入
- `expense` - 支出
- `borrowing_in` - 借入
- `lending_out` - 借出
- `repayment_in` - 还入
- `repayment_out` - 还出

**红冲机制**：
- `isReversal` - 是否为红冲记录
- `reversalOfFlowId` - 冲正的原始流水
- 红冲不删除原记录，创建负数流水抵消

### 2. 账户管理 (AccountService)

管理公司银行账户和现金账户。

**账户类型**：
- `cash` - 现金
- `bank` - 银行
- `digital` - 数字钱包

**余额计算**：
- 期初余额 + SUM(流水金额) = 当前余额
- 使用乐观锁防止并发问题

### 3. 账户转账 (AccountTransferService)

跨账户/跨币种转账。

**支持**：
- 同币种转账
- 跨币种转账（记录汇率）

### 4. 应收应付 (ArApDocService)

应收账款(AR)/应付账款(AP)管理。

**状态**：
- `open` - 待处理
- `partial` - 部分结算
- `settled` - 已结清

---

## 📊 财务报表

### 现金流报表

```
GET /api/v2/reports/cash-flow?startDate=2025-01-01&endDate=2025-12-31
```

### 账户余额报表

```
GET /api/v2/reports/account-balance
```

### 应收账龄分析

```
GET /api/v2/reports/ar-aging
```

---

## 💵 金额处理规范

> [!IMPORTANT]
> 所有金额存储为**整数（分/cents）**，避免浮点数精度问题。

```typescript
// 存储
const amountCents = Math.round(amount * 100)

// 显示
const displayAmount = amountCents / 100
```

---

**最后更新**: 2025-12-27

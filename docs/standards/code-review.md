...
### 状态机验证
- [ ] 状态转换是否使用 `StateMachine.validateTransition()`？
- [ ] 新状态流转是否在 `state-machine.ts` 中定义？
- [ ] 是否处理了终态（不允许转换）的情况？

### 乐观锁并发控制
- [ ] 需要并发控制的操作是否使用 `validateVersion()`？
- [ ] 更新时是否调用 `incrementVersion()`？
- [ ] 是否在 `beforeData` 中记录版本号？

### 事务安全
- [ ] 多表操作是否使用 `db.transaction()`？
- [ ] 事务内是否使用 `.run()` 而非 `.execute()`？
- [ ] 只读查询是否移到事务外？

### 删除操作验证
- [ ] 删除前是否检查记录存在？
- [ ] 是否检查业务状态（如 open/settled）？
- [ ] 是否清理关联数据或重置关联状态？
